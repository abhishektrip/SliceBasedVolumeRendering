#include "atMesh.h"
// Include AssImp
#include "assimp-3.0.1270/include/assimp/Importer.hpp"      // C++ importer interface
#include "assimp-3.0.1270/include/assimp/scene.h"          // Output data structure
#include "assimp-3.0.1270/include/assimp/postprocess.h"     // Post processing flags
...
{
	mScalingMatrix(0, 0) = scaleFactor.x();
	mScalingMatrix(1, 1) = scaleFactor.y();
	mScalingMatrix(2, 2) = scaleFactor.z();
}
...
Matrix4f atRenderEngine::atMeshData::GetTRMatrix()
{
	Matrix4f modelMatrix = (mTranslationMatrix*mRotationMatrix);
	return modelMatrix;
}
...
void atRenderEngine::atMeshData::GetIntersectingTriangleWithRay(Utility::RayData & ray, std::vector<int>& indices)
{
	...
	if (meshCollider->IntersectingBBox(ray, nearest, GetTRMatrix(), Vector3f(mScalingMatrix(0, 0), mScalingMatrix(1, 1), mScalingMatrix(2, 2))) && nearest != nullptr)
	{
		indices = std::vector<int>(nearest->data.elementIdx);
	}
}

Vector3f atMeshData::CalculateNormal(Vector3f v1, Vector3f v2, Vector3f v3)
{
	...
	Vector3f normal = u.cross(v).normalized();
	return normal;
}
			...
			else
			{
				for (int i = 0; i < eqTriCount; i++)
				{
					...
					for (int i = 0; i < numSymmetricParts; i++, degToRot += degToRotStep)
					{
						...
						Vector3f normal = CalculateNormal(tempVertList[count*i + vertIdx[0]], tempVertList[count*i + vertIdx[1]], tempVertList[count*i + vertIdx[2]]);
			...
			else
			{
				for (int i = 0; i < eqTriCount; i++)
				{
					...
					for (int j = 0; j < numSymmetricParts; j++, degToRot +=degToRotStep)
					{
						...
						triData.vertexIdx[0] = count*j + vertIdx[0];
						triData.vertexIdx[1] = count*j + vertIdx[1];
						triData.vertexIdx[2] = count*j + vertIdx[2];

						if (isBarrel && j > 0)
						{
							int idx = 1 - j % 2;
							mUVList[count*j + vertIdx[0]] = uvValues[colorIdx[idx]];
							mUVList[count*j + vertIdx[1]] = uvValues[colorIdx[idx]];
							mUVList[count*j + vertIdx[2]] = uvValues[colorIdx[idx]];

							triData.uvIdx[0] = count*j + vertIdx[0];
							triData.uvIdx[1] = count*j + vertIdx[1];
							triData.uvIdx[2] = count*j + vertIdx[2];
						...
						{
							mUVList[count*j + vertIdx[0]] = uvValues[colorIdx[j]];
							mUVList[count*j + vertIdx[1]] = uvValues[colorIdx[j]];
							mUVList[count*j + vertIdx[2]] = uvValues[colorIdx[j]];
...
bool atMeshData::ReadObjFile(std::string path)
{
	...
	FILE* file = fopen(path.c_str(), "r");
	std::vector<Vector3f>tempNormalList; 
	char prefix[3];   // prefix reader - 3 chars should be sufficient. 
	...
	else
	{
		while (fscanf(file, "%2s", prefix) != EOF)
		{
			...
			else if (strcmp(prefix, "vn") == 0)
			{
				...
				tempNormalList.push_back(normal);
			}
		...
		mUVList.clear();
		mNormalList = std::vector<Vector3f>(NumberOfVertices());
		
		for (int i = 0; i < NumberOfTriangles(); i++)
		{
			mNormalList[mTriangleList[i].vertexIdx[0]] = tempNormalList[mTriangleList[i].normalIdx[0]];
			mNormalList[mTriangleList[i].vertexIdx[1]] = tempNormalList[mTriangleList[i].normalIdx[1]];
			mNormalList[mTriangleList[i].vertexIdx[2]] = tempNormalList[mTriangleList[i].normalIdx[2]];
		}
	...
	if (fd != NULL)
	{
		...
		for (int i = 0; i < NumberOfTriangles(); i++)
		{
			fprintf(fd, "f %d/%d/ %d/%d/ %d/%d/\n",
				mTriangleList[i].vertexIdx[0] + 1, mTriangleList[i].vertexIdx[0]+1,
				mTriangleList[i].vertexIdx[1] + 1, mTriangleList[i].vertexIdx[1]+1,
				mTriangleList[i].vertexIdx[2] + 1, mTriangleList[i].vertexIdx[2]+1);
		}
...
bool atMeshData::loadAssImp(const std::string path)
{
	...
	for (unsigned int meshNum = 0; meshNum < scene->mNumMeshes; meshNum++)
	{
		...
		aiMesh* mesh = scene->mMeshes[meshNum]; // In this simple example code we always use the 1rst mesh (in OBJ files there is often only one anyway)

												// Fill vertices positions
		//mVertexList.reserve(mesh->mNumVertices);
		for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
			aiVector3D pos = mesh->mVertices[i];
			mVertexList.push_back(Vector3f(pos.x, pos.y, pos.z));
		}

		// Fill vertices texture coordinates
		//mUVList.reserve(mesh->mNumVertices);
		...
		else
		{
			for (unsigned int i = 0; i < mesh->mNumVertices; i++)
			{
				...
				mUVList.push_back(Vector2f(UVW.x, UVW.y));
			}
		}

		// Fill vertices normals
		//mNormalList.reserve(mesh->mNumVertices);
		for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
			aiVector3D n = mesh->mNormals[i];
			mNormalList.push_back(Vector3f(n.x, n.y, n.z));
		}
		...
		{
			Utility::TriangleData triData;
			// Assume the model has only triangles.
			/*indices.push_back(mesh->mFaces[i].mIndices[0]);
			indices.push_back(mesh->mFaces[i].mIndices[1]);
			indices.push_back(mesh->mFaces[i].mIndices[2]);*/

			triData.vertexIdx[0] = triData.normalIdx[0] = triData.uvIdx[0] = verticesAdded + mesh->mFaces[i].mIndices[0];
			triData.vertexIdx[1] = triData.normalIdx[1] = triData.uvIdx[1] = verticesAdded + mesh->mFaces[i].mIndices[1];
			triData.vertexIdx[2] = triData.normalIdx[2] = triData.uvIdx[2] = verticesAdded + mesh->mFaces[i].mIndices[2];

			mTriangleList.push_back(triData);

		}
		verticesAdded = mVertexList.size();
	}
