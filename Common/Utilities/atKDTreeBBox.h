...
namespace atRenderEngine
{
	...
	namespace Utility
	{
		...
		public:
			...
			bool IntersectingBBox(const Utility::RayData & x, atKDNodeBBox *& nearest, Matrix4f & modelMatrix, Vector3f& scaleFactor);
			void IntersectingBBox(atKDNodeBBox * topNode, const Utility::RayData & x, atKDNodeBBox *& nearest, Matrix4f& modelMatrix, Vector3f& scaleFactor, std::function<bool(atKDNodeBBox*)> conditionOperator, bool& found, int& depth , float& distance);
			atKDTreeBBox();
			~atKDTreeBBox();
			void Draw(Matrix4f&);
