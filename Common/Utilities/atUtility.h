...
namespace atRenderEngine
{
	namespace Utility
	{
		...
		struct TransferFunction1DPoint
		{
			Vector4f colorValue;
			int sampleValue;

			TransferFunction1DPoint(Vector3f color, int sample)
				: colorValue(Vector4f(color.x(),color.y(),color.z(),1.0f)),sampleValue(sample)
			{}

			TransferFunction1DPoint(float alpha, int sample)
				: colorValue(Vector4f(0.0f, 0.0f, 0.0f, alpha)), sampleValue(sample)
			{}
		};
...
/*			transMatrix(0, 0) += cos(Utility::ToRadians(degree));
		...
		//void convertToRGBA(char *& dest, std::vector<Vector4f>& src, int size);
		void DrawCoordinateAxes(Matrix4f& currentMatrix);
