...
void Utility::atKDTreeBBox::insert(std::vector<int>& indices, atMeshData& mesh, atKDNodeBBox* t, int cd , int depth)
{
	...
	else
	{
		...
		for (int i = 0; i < indices.size(); i++)
		{
			...
			BoundingBoxContains IsContained = Utility::IsWithinBoundingBox(leftBBox, v , cd);
			...
			IsContained = Utility::IsWithinBoundingBox(rightBBox, v , cd);
			if (IsContained == BoundingBoxContains::BB_FULLY_WITHIN || IsContained == BoundingBoxContains::BB_PARTIALLY_WITHIN)
			{				
...
void Utility::atKDTreeBBox::BuildTree( atMeshData& mesh)
{
	...
	root->name = "Root";
	insert(listOfIndices, mesh, root,1 , 0);

	programID = Utility::atLoadShaders("../Common/Shaders/SimpleColor.vs", "../Common/Shaders/SimpleColor.fs");
}

bool Utility::atKDTreeBBox::IntersectingBBox(const Utility::RayData& x, atKDNodeBBox*& nearest, Matrix4f& modelMatrix, Vector3f& scaleFactor )
{
	bool found = false; 
	int depth = -1; 
	float distance;
	IntersectingBBox(root, x, nearest, modelMatrix,scaleFactor, nullptr, found, depth, distance);
	if (found)
	{
...
void Utility::atKDTreeBBox::IntersectingBBox(atKDNodeBBox* topNode, const Utility::RayData& x, atKDNodeBBox*& nearest,Matrix4f& modelMatrix, Vector3f& scaleFactor , std::function<bool(atKDNodeBBox*)> conditionOperator, bool& found, int& depth , float& distance )
{
	if (topNode == nullptr)
		return;
	if (&x == nullptr)
	...
	std::cout << " BBox Name : " << topNode->name << std::endl;
	if ((conditionOperator == nullptr || conditionOperator(topNode))&& 
		Utility::TestRayOBBIntersection(x.rayOrigin, x.rayDirection, topNode->data.bbox.min.cwiseProduct(scaleFactor), topNode->data.bbox.max.cwiseProduct(scaleFactor), modelMatrix, distance))
	{
		std::cout << "Found Intersection for " << topNode->name << std::endl;
		nearest = topNode; 
		found = true; 
		depth = depth + 1;
		atKDNodeBBox* tempNodeLeft = nullptr, *tempNodeRight = nullptr;

		IntersectingBBox(topNode->leftTree, x, tempNodeLeft, modelMatrix, scaleFactor, conditionOperator, found,depth, distLeft);
		IntersectingBBox(topNode->rightTree, x, tempNodeRight, modelMatrix,scaleFactor, conditionOperator, found,depth , distRight);
